from Director import Director, Team, DirectorActions, DirectorBeliefs, DirectorDesires
from Pilots import Pilot, PilotActions, PilotBeliefs, PilotDesires
from Driving import DrivingStyle, GeneticStrategySelection, Car
from Message import SistemaMensajeria
from Mechanics import Mechanic, MechanicActions, MechanicBeliefs, MechanicDesires, PitStop
from Tires import Tire, C0Tire, C1Tire, C2Tire, C3Tire, GreenIntermediateTire,FullWetBlueTire, TireAdaptationSystem
from Race import Race, Track,  Segment, Weather, Difficulty
import time
import pprint
import random
from Tournament import Torneo

race = Race.create_race()
#pprint.pprint(vars(race))

random_director = Director.generate_random_director(race)
#pprint.pprint(vars(random_director))
...
from utils import fitness, strategy_factor, car_factor
...
def race_sim(directors, pilots, race, deseos):
    print("El largo de la pista es "+ str(race.track.length) + " y la cantidad de vueltas "+str(race.no_laps))
    ...
    while True:
        ...
        for i in range(0, len(pilots)):
           print("El piloto " + pilots[i].name + " va a una velocidad de " + str(velocidades_actuales[i]))
    ...
    print(f"La carrera duró {race.duration *100} segundos.")
    print(final_times)
    for i in range(len(pilots)):
...
def redirect_stdout_to_file(file_path):
    ...
    with open(file_path, 'w') as file:
        ...
        # Aquí puedes imprimir lo que quieras y se guardará en el 
        race = Race.create_race()
        directors = generate_n_random_directors(race)
        for dir in directors:
           race.equipos.append(dir.equipo.name)
